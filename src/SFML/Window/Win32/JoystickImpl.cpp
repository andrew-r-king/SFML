...
namespace
{
    ...
    const sf::Time connectionRefreshDelay = sf::Time::milliseconds(500);
...
JoystickState JoystickImpl::update()
{
    ...
    if (joyGetPosEx(m_index, &pos) == JOYERR_NOERROR)
    {
        ...
        if (pos.dwPOV != 0xFFFF)
        {
            float angle = pos.dwPOV / 18000.f * Math::pi_v<float>;
        ...
        for (int j = 0; j < Joystick::AxisCount; ++j)
        {
            if (m_axes[j] == events[i].dwOfs)
            {
                if ((j == Joystick::PovX) || (j == Joystick::PovY))
                {
                    ...
                    if (value != 0xFFFF)
                    {
                        float angle = (static_cast<float>(value)) * Math::pi_v<float> / DI_DEGREES / 180.f;
